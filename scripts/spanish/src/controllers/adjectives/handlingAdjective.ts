...
export async function handlingAdjective(word: Word) {
  try {
    ...
    if (existsInDatabase) {
      console.log(
        `${chalk.yellow("⚠️ ", adjective.singular_masculine, " already exists in database\n⚠️ proceeding to next word\n")}`,
      );
      return;
    } else {
      return false;
    ...
    if (!isInWiktionary) {
      console.log(
        `${chalk.red("❌ ", adjective.singular_masculine, " not found in wiktionary, exiting...\n")}`,
      );
      return;
    } else {
      return false;
    ...
    if (!inflectionsFound) {
      console.log(
        `${chalk.red("❌ no inflections found for ", adjective.singular_masculine, ", exiting...\n")}`,
      );
      return;
    } else {
      return false;
